-- Athena V18 — Organization-level Ads campaigns + durable Ads generation jobs.
-- Additive only. Does not alter existing tables, job contracts, or foreign keys.
-- Ads belong to organizations; never to discussions, prospects, personas, or executive versions.
...
create index if not exists ad_campaigns_org_created_idx
  on ad_campaigns (organization_id, created_at desc);

create index if not exists ad_campaigns_org_status_idx
  on ad_campaigns (organization_id, status);

comment on table ad_campaigns is
  'Organization-level advertising campaigns. Owned by organization only — not discussions, prospects, personas, or executive versions.';
comment on column ad_campaigns.organization_id is
  'Trusted organization owner. Ads are organization-scoped, never discussion-scoped.';
comment on column ad_campaigns.brief_json is
  'Optional operator guidance. Empty object means inferred campaign mode.';
comment on column ad_campaigns.package_json is
  'Complete validated AdCampaignPackage. Present only when status is Ready.';
comment on column ad_campaigns.status is
  'Queued | Processing | Ready | Processing Failed';

-- ---------------------------------------------------------------------------
-- athena_ad_generation_jobs
-- ---------------------------------------------------------------------------
create table if not exists athena_ad_generation_jobs (
  id uuid primary key default gen_random_uuid(),
...
create index if not exists athena_ad_generation_jobs_org_idx
  on athena_ad_generation_jobs (organization_id);

create index if not exists athena_ad_generation_jobs_campaign_idx
  on athena_ad_generation_jobs (campaign_id, created_at desc);

create index if not exists athena_ad_generation_jobs_claimable_idx
  on athena_ad_generation_jobs (status, next_attempt_at, created_at);

create index if not exists athena_ad_generation_jobs_lease_idx
  on athena_ad_generation_jobs (status, claim_expires_at)
  where status = 'processing';

-- One active Ads generation job per campaign
create unique index if not exists athena_ad_generation_jobs_one_active_campaign
  on athena_ad_generation_jobs (campaign_id)
  where status in ('queued', 'processing', 'retryable');

comment on table athena_ad_generation_jobs is
  'Durable organization-level Ads campaign generation jobs. Independent of athena_generation_jobs and deep scrape jobs.';

-- ---------------------------------------------------------------------------
-- Claim / heartbeat / complete / fail RPCs (service-role only)
-- ---------------------------------------------------------------------------
create or replace function claim_athena_ad_generation_job(
  p_worker_id text,
  p_claim_token uuid,
...
begin
  ...
  -- Mirror campaign processing state (organization-scoped via campaign FK)
  update ad_campaigns
  set
    status = 'Processing',
...
create or replace function heartbeat_athena_ad_generation_job(
  p_job_id uuid,
  p_claim_token uuid,
  p_lease_seconds integer default 120,
