/**
 * Presentation-only grouping of already-parsed deployment assets.
 * Does not rename keys, alter packages, or invent assets.
 */

export type GroupableDeploymentAsset = {
  assetKey?: string;
  title: string;
  objective: string;
  content: string;
...
function normalizeAssetIdentity(asset: GroupableDeploymentAsset): string {
...
function hasContent(asset: GroupableDeploymentAsset): boolean {
  return Boolean(asset.content?.trim());
}

export function groupDeploymentOutreachAssets<
  T extends GroupableDeploymentAsset,
>(assets: readonly T[]): { outreach: T[]; other: T[] } {
  const outreach: T[] = [];
  const other: T[] = [];
...
export function findDeploymentAssetByKeys<T extends GroupableDeploymentAsset>(
  assets: readonly T[],
