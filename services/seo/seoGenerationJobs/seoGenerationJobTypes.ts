/**
 * Durable SEO generation job types (independent of athena_generation_jobs).
 */

import type { SeoReportGenerationStage } from "@/services/seo/seoReportTypes";

export const SEO_GENERATION_JOB_STATUSES = [
  "queued",
  "processing",
  "completed",
  "failed",
...
export type SeoGenerationJobStatus = (typeof SEO_GENERATION_JOB_STATUSES)[number];

export const SEO_GENERATION_JOB_RETRY_BACKOFF_MS = [30_000, 120_000] as const;

export type AthenaSeoGenerationJob = {
  id: string;
  organization_id: string;
  report_id: string;
  status: SeoGenerationJobStatus;
  generation_stage: SeoReportGenerationStage | string | null;
...
export function isActiveSeoGenerationJobStatus(status: string): boolean {
  return status === "queued" || status === "processing" || status === "retryable";
}

export function mapSeoGenerationJobRow(
  row: Record<string, unknown>,
): AthenaSeoGenerationJob {
  return {
    id: String(row.id),
    organization_id: String(row.organization_id),
    report_id: String(row.report_id),
    status: row.status as SeoGenerationJobStatus,
